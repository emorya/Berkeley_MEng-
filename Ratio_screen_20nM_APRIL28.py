...
metadata = {
    "protocolName": "April 20th: Ratio Screen — 20 nM scaffold (1:1 / 1:2.5 / 1:3.5 / 1:5)",
    "author": "TiLab",
    "description": (
        "Scaffold:staple ratio screen for 20 nM scaffold. "
        "p300 builds pools and condition tubes. p20 handles TC transfer and gel loading."
    ),
}

requirements = {"robotType": "OT-2", "apiLevel": "2.26"}
OFFSET = {"x":-1.5, "y": -2.5, "z": 16.5}
y_diff = -9
...
#
# Scaffold 84 nM working stock (tube A2):
    # 40 uL of 420 nM M13 stock + 160 uL 1x TAE = 200 uL at 84 nM
    # (1x TAE: 20 uL of 50x TAE + 980 uL MQ water)
    # Pipette vol per reaction: 10 uL -> 20 nM final in 42 uL
#
# MQ water (tube A3):
...
CONDITION_VOLS = [
    (20, "1:1", 20, "master",  3.77, 24.03),  # staple from master (223 nM)
    (20, "1:2.5", 50, "master",  9.42, 18.38),  # staple from master (223 nM)
    (20, "1:3.5", 70, "master", 13.18, 14.62),  # staple from master (223 nM)
    (20, "1:5",  100, "master", 18.83,  8.97),  # staple from master (223 nM)
    ]


CONDITION_TUBE_WELLS = ['B1','B2','B3','B4']
MASTER_MIX_VOL = 14.20   # uL per condition (4.20 buffer + 10.00 scaffold)
TOTAL_VOL = 42.0
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    p20.starting_tip = tiprack_20a['B2']
    ...
    tc_dest_wells   = [tc_plate[f"A{i}"] for i in range(1,5)]
...
#     # ==================================================================
    # protocol.pause("Open the p300 and p20 tips") 
...
#     # # ==================================================================
    ...
    gel_lanes = ['C1','D1','E1','F1']
    sample_uL = 14 #changed from 16 because I think it'll dispense better this way
    ladder_uL = 5.0
    ...
    # p20.pick_up_tip()
    # p20.move_to(minione_plate["A1"].top(),speed=Z_SLOW)
    # protocol.pause()
    # p20.move_to(minione_plate["A1"].top(-3),speed=Z_SLOW)
    # protocol.pause()
    # p20.move_to(minione_plate["C1"].top(-3),speed=Z_SLOW)
    # protocol.pause()
    # p20.move_to(minione_plate["E1"].top(-3),speed=Z_SLOW)
